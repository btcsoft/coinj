...
public class CoinURI {
     ...
     *
     * @throws CoinURIParseException If the input fails Bitcoin URI syntax and semantic checks.
     */
    public CoinURI(@Nullable NetworkParameters params, String input) throws CoinURIParseException {
        checkNotNull(input);
        ...
        String prefix = "";
        final ImmutableCollection<CoinDefinition> registeredCoins = CoinLocator.getRegisteredCoins();
        boolean hit = false;
        String foundUriScheme = null;
        for (final CoinDefinition coin : registeredCoins) {
            foundUriScheme = coin.getUriScheme();
            final String prefix1 = foundUriScheme + "://";
            final String prefix2 = foundUriScheme + ":";
            if (input.startsWith(prefix1)) {
                schemeSpecificPart = input.substring(prefix1.length());
                prefix = prefix1;
                hit = true;
                break;
            } else if (input.startsWith(prefix2)) {
                schemeSpecificPart = input.substring(prefix2.length());
                prefix = prefix2;
                hit = true;
                break;
            }
        }
        if (!hit || foundUriScheme == null) {
            throw new CoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        uriScheme = foundUriScheme;

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?", 2);
        if (addressSplitTokens.length == 0)
            throw new CoinURIParseException("No data found after the " + prefix + ": prefix");
        ...
        parseParameters(params, addressToken, nameValuePairTokens, prefix);
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens, String prefix) throws CoinURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new CoinURIParseException("Malformed " + prefix + " URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new CoinURIParseException("Malformed " + prefix + " URI - empty name '" +
                        nameValuePairToken + "'");
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder(uriScheme.substring(0, 1).toUpperCase());
        builder.append(uriScheme.substring(1));
        builder.append("URI[");
        boolean first = true;
    ...
    public static String convertToBitcoinURI(Address address, CoinDefinition def, Coin amount, String label, String message) {
        return convertToBitcoinURI(address.toString(), def, amount, label, message);
    }
     ...
     */
    public static String convertToBitcoinURI(String address, CoinDefinition def, @Nullable Coin amount, @Nullable String label, @Nullable String message) {
        ...
        builder.append(def.getUriScheme()).append(":").append(address);
